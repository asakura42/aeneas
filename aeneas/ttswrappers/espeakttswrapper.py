...
class ESPEAKTTSWrapper(BaseTTSWrapper):
    ...
    CODE_TO_HUMAN_LIST = sorted(["%s\t%s" % (k, v) for k, v in CODE_TO_HUMAN.items()])
    ...
    def __init__(self, rconf=None, logger=None):
        ...
        # Optionally disable end pause at fragment end (espeak CLI -z)
        try:
            if self.rconf["espeak_cli_no_endpause"]:
                # insert after "-b", "1"
                args.insert(3, "-z")
        except Exception:
            # Key not present or rconf not subscriptable; ignore
            pass
