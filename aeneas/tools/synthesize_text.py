...
class SynthesizeTextCLI(AbstractCLIProgram):
    ...
    HELP = {
        ...
        "examples": [
            ...
            "mplain %s eng %s" % (TEXT_FILE_MPLAIN, OUTPUT_FILE),
            "munparsed %s eng %s --l1-id-regex=p[0-9]+ --l2-id-regex=s[0-9]+ --l3-id-regex=w[0-9]+"
            % (TEXT_FILE_MUNPARSED, OUTPUT_FILE),
            "parsed %s eng %s" % (TEXT_FILE_PARSED, OUTPUT_FILE),
            "plain %s eng %s" % (TEXT_FILE_PLAIN, OUTPUT_FILE),
            "subtitles %s eng %s" % (TEXT_FILE_SUBTITLES, OUTPUT_FILE),
            "unparsed %s eng %s --id-regex=f[0-9]*" % (TEXT_FILE_UNPARSED, OUTPUT_FILE),
            "unparsed %s eng %s --class-regex=ra" % (TEXT_FILE_UNPARSED, OUTPUT_FILE),
            "unparsed %s eng %s --id-regex=f[0-9]* --sort=numeric"
            % (TEXT_FILE_UNPARSED, OUTPUT_FILE),
            "plain %s eng %s --start=5" % (TEXT_FILE_PLAIN, OUTPUT_FILE),
            "plain %s eng %s --end=10" % (TEXT_FILE_PLAIN, OUTPUT_FILE),
            "plain %s eng %s --start=5 --end=10" % (TEXT_FILE_PLAIN, OUTPUT_FILE),
            "plain %s eng %s --backwards" % (TEXT_FILE_PLAIN, OUTPUT_FILE),
            "plain %s eng %s --quit-after=10.0" % (TEXT_FILE_PLAIN, OUTPUT_FILE),
        ],
        "options": [
