...
from aeneas.syncmap import SyncMapFormat, SyncMapFragment, SyncMapHeadTailFormat
...
class ExecuteTaskCLI(AbstractCLIProgram):
    ...
    def perform_command(self):
        ...
        if print_zero:
            zero_duration = [
                l
                for l in task.sync_map_leaves(SyncMapFragment.REGULAR)
                if l.begin == l.end
            ]
        ...
        if print_rates:
            ...
            for fragment in task.sync_map_leaves(SyncMapFragment.REGULAR):
                self.print_generic(
                    "  %s\t%.3f" % (fragment.pretty_print, fragment.rate or 0.0)
                )
    ...
    def print_examples(self, full=False):
        ...
        for key in sorted(self.DEMOS.keys()):
            ...
            if full or example["show"]:
                ...
                msg.append("  $ %s %s" % (self.invoke, key))
                msg.append("")
                i += 1
        self.print_generic("\n" + "\n".join(msg) + "\n")
